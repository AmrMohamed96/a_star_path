    ...
    for k in range(-1, 2):
        for j in range(-1, 2):
            if (k != j or k != 0):
                s_x = node_x + k
                s_y = node_y + j
                # the following if condition happens one time
...
rospy.init_node('Planning_3', anonymous=False)
pub = rospy.Publisher('Planning_Output3', Int32MultiArray)
...
while map.size == 1:
    rospy.Subscriber('rob3_map', Int32MultiArray, map_subscriber)
    rospy.sleep (2)
...
# that step is extremly required >>>>>>>>>>>>.... don't forget it
